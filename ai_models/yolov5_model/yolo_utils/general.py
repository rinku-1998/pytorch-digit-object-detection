...
DATASETS_DIR = Path(os.getenv('YOLOv5_DATASETS_DIR', ROOT.parent /
                              'datasets'))  # global datasets directory
AUTOINSTALL = str(os.getenv(
    'YOLOv5_AUTOINSTALL', True)).lower() == 'true'  # global auto-install mode
VERBOSE = str(os.getenv('YOLOv5_VERBOSE',
                        True)).lower() == 'true'  # global verbose mode
...
os.environ[
    'TF_CPP_MIN_LOG_LEVEL'] = '2'  # suppress verbose TF compiler warnings in Colab
...
if platform.system() == 'Windows':
    for fn in LOGGER.info, LOGGER.warning:
        setattr(LOGGER, fn.__name__,
                lambda x: fn(emojis(x)))  # emoji safe logging
...
class Timeout(contextlib.ContextDecorator):
    ...
    def __enter__(self):
        if platform.system() != 'Windows':  # not supported on Windows
            ...
            signal.alarm(
                self.seconds)  # start countdown for SIGALRM to be raised
...
def check_git_status(repo='ultralytics/yolov5', branch='master'):
    ...
    assert Path(
        '.git').exists(), s + 'skipping check (not a git repository)' + msg
    assert check_online(), s + 'skipping check (offline)' + msg

    splits = re.split(pattern=r'\s',
                      string=check_output('git remote -v',
                                          shell=True).decode())
    ...
    local_branch = check_output('git rev-parse --abbrev-ref HEAD',
                                shell=True).decode().strip()  # checked out
    n = int(
        check_output(f'git rev-list {local_branch}..{remote}/{branch} --count',
...
def check_git_info(path='.'):
    ...
    try:
        ...
        remote = repo.remotes.origin.url.replace(
            '.git', '')  # i.e. 'https://github.com/ultralytics/yolov5'
...
def check_file(file, suffix=''):
    ...
    if os.path.isfile(file) or not file:  # exists
        ...
        if os.path.isfile(file):
            LOGGER.info(
                f'Found {url} locally at {file}')  # file already exists
        else:
...
def check_dataset(data, autodownload=True):
    ...
    extract_dir = ''
    if isinstance(data,
                  (str, Path)) and (is_zipfile(data) or is_tarfile(data)):
        download(data,
...
def check_amp(model):
    ...
    def amp_allclose(model, im):
        ...
        return a.shape == b.shape and torch.allclose(
            a, b, atol=0.1)  # close to 10% absolute tolerance
...
def download(url,
    ...
    def download_one(url, dir):
        ...
        if unzip and success and (f.suffix == '.gz' or is_zipfile(f)
                                  or is_tarfile(f)):
            LOGGER.info(f'Unzipping {f}...')
            if is_zipfile(f):
                unzip_file(f, dir)  # unzip
            elif is_tarfile(f):
                subprocess.run(['tar', 'xf', f, '--directory', f.parent],
                               check=True)  # unzip
            elif f.suffix == '.gz':
...
if Path(inspect.stack()
        [0].filename).parent.parent.as_posix() in inspect.stack()[-1].filename:
